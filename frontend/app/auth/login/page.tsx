        ...
        <div className='flex-col items-center justify-center w-full p-4 md:w-2/3'>
          ...
          <form onSubmit={(e) => {
            ...
            <div>
              ...
              <input required disabled={loading ? true : false} className='w-full mt-1 auth-input-text' type="text" name='login' placeholder='Wpisz swój login do konta librus' />
            </div>
            <div className='mt-4'>
              <label className='auth-input-label' htmlFor="password">Hasło</label> <br />
              <input required disabled={loading ? true : false} className='w-full mt-1 auth-input-text' type="password" name='password' placeholder='Wpisz swoje hasło do konta librus' />
            </div>
            <div className='mt-2 text-sm text-red-500'>{ error }&nbsp;</div>
            <button disabled={loading ? true : false} className='mt-6 auth-button-submit' type="submit">
              {loading ? (
