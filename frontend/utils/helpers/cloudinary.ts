...
export const uploadImages = async (images: []): Promise<string[]> => {
  ...
  for (const image in images) {
    ...
    data.append("api_secret", process.env.NEXT_PUBLIC_CLOUDINARY_API_SECRET as string);
    data.append("cloud_name", process.env.NEXT_PUBLIC_CLOUDINARY_NAME as string);
