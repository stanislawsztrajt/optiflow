...
class EventsServices {
  async create (lostItem: IlostItem): Promise<IlostItem> {
    const { data }: Iresponse<IlostItem> = await axios.post(url, lostItem, authHeader)
    return data
  }
  ...
  async update (lostItem: IlostItem, id: string): Promise<IlostItem> {
    const { data }: Iresponse<IlostItem> = await axios.patch(`${url}/${id}`, lostItem, authHeader)
    return data
  }

  async remove (id: string): Promise<IlostItem> {
    const { data }: Iresponse<IlostItem> = await axios.delete(`${url}/${id}`, authHeader)
    return data
  }
}
