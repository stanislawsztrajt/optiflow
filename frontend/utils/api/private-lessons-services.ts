...
class EventsServices {
  async create (privateLesson: IprivateLesson): Promise<IprivateLesson> {
    const { data }: Iresponse<IprivateLesson> = await axios.post(url, privateLesson, authHeader)
    return data
  }
  ...
  async update (privateLesson: IprivateLesson, id: string): Promise<IprivateLesson> {
    const { data }: Iresponse<IprivateLesson> = await axios.patch(`${url}/${id}`, privateLesson, authHeader)
    return data
  }

  async remove (id: string): Promise<IprivateLesson> {
    const { data }: Iresponse<IprivateLesson> = await axios.delete(`${url}/${id}`, authHeader)
    return data
  }
}
